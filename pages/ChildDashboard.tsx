...
export const ChildDashboard: React.FC = () => {
    const { assignedLessons, setActiveLessonId, childProfile, lessonHistory, backgroundSession, resumeSession, curriculumStats, dailyQuote, availableBadges, isLoading } = useApp();
    ...
    if (!childProfile) {
        return (
            <div className="min-h-screen bg-[#0b1527] flex flex-col items-center justify-center text-white p-6 text-center">
                <div className="text-4xl mb-4">⚠️</div>
                <h2 className="text-2xl font-bold mb-2">Profile Not Found</h2>
                <p className="text-scholafy-muted max-w-md mb-6">
                    We couldn't load or create your profile. This usually means the database permissions need a quick fix.
                </p>
                <div className="bg-black/20 p-4 rounded-xl border border-white/10 text-left text-sm font-mono text-blue-300 mb-6 w-full max-w-lg">
                    1. Go to Supabase Dashboard &gt; SQL Editor<br />
                    2. Run: <span className="text-white select-all">CREATE POLICY "Users can insert own profile" ON profiles FOR INSERT WITH CHECK (auth.uid() = id);</span>
                </div>
                <button
                    onClick={() => window.location.reload()}
                    className="px-6 py-2 bg-scholafy-accent text-scholafy-navy font-bold rounded-lg hover:bg-yellow-400 transition-colors"
                >
                    Reload Page
                </button>
            </div>
